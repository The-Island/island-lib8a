...
exports.searchUser = function(name, cb) {
  ...
  Step(
    ...
    function getPostHeaders() {
      request.get({
        uri: 'https://www.8a.nu/scorecard/Search.aspx',
...
exports.getTicks = function(userId, cb) {
  Step(
    function getJavascripts() {
      ...
      request.get({
        uri: 'https://www.8a.nu/js/u_common.aspx?UserId=' + userId,
        headers: _.extend(headers,
            {'Referer': 'https://www.8a.nu/user/Profile.aspx?UserId' + userId}),
        encoding: 'binary',
      }, this);
    },
    function getAscents(err, res, body) {
      ...
      request.get({
        uri: 'https://www.8a.nu/js/sc_common.aspx?GID=' + gid + '&UserId=' +
            userId,
        headers: _.extend(headers,
            {'Referer': boulderUrl}),
        encoding: 'binary',
    ...
    function parseHtmls(err, boulders, routes, javascript) {
      ...
      // Ensure user is public.
      var anonymousUser_rx = new RegExp(/This user is anonymous\./);
      if (boulders.body.match(anonymousUser_rx) ||
          routes.body.match(anonymousUser_rx)) {
        return cb({code: 403, message: 'Cannot access anonymous user',
            explain: 'Please make sure your 8a.nu user is not deactivated or' +
            ' set to "Anonymous".'});
      }
